...
impl<A> ArrayRead<[bool]> for DataRef<[bool], A> {
    fn get_subdata(&self, dest: &mut [bool], start_offset: usize) -> usize {
        let mut tmp_buf = vec![0; dest.len()];
        let in_len = unsafe {
            XPLMGetDatavi(
                self.id,
                tmp_buf.as_mut_ptr(),
        ...
        let act_len = in_len.min(tmp_buf.len());
        dest[..act_len]
            .iter_mut()
            .enumerate()
            .for_each(|(idx, value)| {
    ...
    fn len(&self) -> usize {
        let size = unsafe { XPLMGetDatavi(self.id, ptr::null_mut(), 0, 0) };
        size as usize
    }
}

impl ArrayReadWrite<[bool]> for DataRef<[bool], ReadWrite> {
    fn set_subdata(&mut self, values: &[bool], start_offset: usize) {
        let mut int_values = values.iter().map(|v| *v as i32).collect::<Vec<_>>();
        unsafe {
            XPLMSetDatavi(
                self.id,
